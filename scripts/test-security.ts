import { compare as verifyPassword, hash as hashPassword } from 'bcryptjs';
...
// ログ出力用の設定
const log = {
  info: (message: string) => console.log(colors.blue(`[INFO] ${message}`)),
  success: (message: string) => console.log(colors.green(`[SUCCESS] ${message}`)),
  warning: (message: string) => console.log(colors.yellow(`[WARNING] ${message}`)),
...
async function testPasswordHashing() {
  ...
  try {
    // ハッシュ化
    const hashedPassword = await hashPassword(testPassword, 10);
